...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, isPurchased, onBack, onAddToCart, onPreview }) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div className="flex flex-col gap-4">
            <div className="w-full aspect-[4/5] bg-[#EBE7DE] overflow-hidden">
              {product.imageUrl === 'COVER_COMPONENT' ? (
                <BookCover className="animate-fade-in-up" />
              ) : (
                <img 
               ...
               <div className="flex flex-col sm:flex-row gap-4">
                 {isPurchased ? (
                   <button 
                     ...
                     className="flex-1 py-5 bg-[#2C2A26] text-[#F5F2EB] uppercase tracking-widest text-sm font-medium hover:bg-[#433E38] transition-colors"
                   >
                     Read Full Book
                   </button>
                 ) : (
                   <>
                     ...
                     <button 
                       onClick={() => onPreview(product)}
                       className="flex-1 py-5 bg-transparent border border-[#2C2A26] text-[#2C2A26] uppercase tracking-widest text-sm font-medium hover:bg-[#2C2A26] hover:text-[#F5F2EB] transition-colors"
                     >
                       Preview Book
                     </button>
                   </>
