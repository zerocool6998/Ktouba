...
const Navbar: React.FC<NavbarProps> = ({ onNavClick, cartCount, onOpenCart, user, onLogin, onLogout }) => {
      ...
      >
        <div className="max-w-[1800px] mx-auto px-8 flex items-center justify-between">
          ...
          <div className={`flex items-center gap-6 z-50 relative transition-colors duration-500 ${textColorClass}`}>
            {user ? (
              <button onClick={onLogout} className="text-sm font-medium uppercase tracking-widest hover:opacity-60 transition-opacity hidden sm:block">
                Logout
              </button>
            ) : (
              <button onClick={onLogin} className="text-sm font-medium uppercase tracking-widest hover:opacity-60 transition-opacity hidden sm:block">
                Login
              </button>
            )}
            <button 
